"""Plot primitives, copied verbatim from the reference implementation
(`_legacy/patient/plotting.py`) so that regenerated figures keep exactly the
styling of the ones in the paper: the same colour schemes, marker sequence,
bar/line/KDE construction, axis and legend helpers.

Treat this module as fixed. `scripts/make_figures.py` supplies the same
formatting dicts the reference plotting notebook used; changes to house style
...
def plot_line(ax,x_values,y_values,y_confidence,labels,formatting):
    ...
    for i in range(len(x_values)):
        ...
        ax.plot(x_values[i],y_values[i],label=labels[i],linewidth=linewidth,color=colors[i],linestyle=linestyle,marker=marker,markevery=formatting.get('markevery', None))#,linestyle=linestyle)
